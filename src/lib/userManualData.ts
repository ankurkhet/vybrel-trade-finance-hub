...
`
  ...
  {
    id: "originator-strategic-matrix",
    title: "6. Strategic Funder Matrix & Pricing",
    category: "originator",
    roles: ["originator_admin"],
    content: `
# Strategic Funder Matrix & Pricing
Originators can now manage lenders using a **3-Product Strategic Rate Matrix**.

## Configuration Steps
**Action:** Navigate to 'Lender Management' and click 'Manage Terms' for a Funder.
**Expected Result:** A dialog appears with inputs for Receivable Purchase, Reverse Factoring, and Payable Finance margins.

## Key Features:
- **Benchmark Selection**: Select SOFR, SONIA, or EURIBOR.
- **Live Market Feed**: View real-time rates fetched via automated Edge Functions.
- **Cascade Updates**: Automatically sync new rates to all active borrower facilities.
`
  },
  {
    id: "funder-kyc-audit",
    title: "7. Funder Compliance: KYC Audit workflow",
    category: "originator",
    roles: ["originator_admin", "account_manager"],
    content: `
# Funder Compliance: KYC Audit workflow
Ensure all participating lenders meet institutional compliance standards.

## Audit Steps
**Action:** Navigate to 'Documents' > 'Funder Audit'.
**Expected Result:** A list of lenders and their submitted KYC packets (Tax IDs, AML Policies).

## Decision Logic:
- Review the uploaded documents.
- Use the **Approve** (Checkmark) or **Reject** (X) icons to record your compliance decision.
- All actions are tracked in the platform's immutable audit log.
`
  },
  {
