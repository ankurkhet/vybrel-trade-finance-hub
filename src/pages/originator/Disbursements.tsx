...
export default function Disbursements() {
    ...
    if (selectedFacility && selectedInvoice) {
      ...
      if (fac && inv) {
        ...
        const applyRates = async () => {
          ...
          const feeConf = (feeConfigs as any[])?.[0];
          ...
          if (feeConf) {
            toast.info(`Fees auto-applied from ${inv.product_type?.replace(/_/g, " ")} fee configuration`);
          }
        };
        applyRates();
      }
  ...
  const handleCreateDisbursement = async () => {
    ...
    if (selectedFacility) {
      const fac = approvedFacilities.find(f => f.id === selectedFacility);
      if (fac && fac.currency && inv.currency && fac.currency !== inv.currency) {
        toast.error(`Currency mismatch: Invoice is in ${inv.currency} but the selected facility is in ${fac.currency}. Please select a matching facility or convert the invoice.`);
        return;
      }
    }
    ...
    // 1. Fetch relevant limits: borrower+counterparty AND borrower-only (two safe queries)
    const [{ data: limitsSpecific }, { data: limitsGlobal }] = await Promise.all([
      // Tier 1: Borrower + specific counterparty limit
      counterparty
        ? supabase.from("funder_limits").select("*").eq("status", "approved").eq("borrower_id", borrowerId).eq("counterparty_name", counterparty)
        : Promise.resolve({ data: [] as any[] }),
      // Tier 2: Borrower-only limit (counterparty_name IS NULL)
      supabase.from("funder_limits").select("*").eq("status", "approved").eq("borrower_id", borrowerId).is("counterparty_name", null),
    ]);
    const limitsData = [...(limitsSpecific || []), ...(limitsGlobal || [])];
    ...
    if (limitsData && limitsData.length > 0) {
      ...
      const totalDisbursed = (disbs || []).reduce((sum, d) => sum + Number((d as any).disbursement_amount || 0), 0);
      const totalRepaid = (repays || []).reduce((sum, r) => sum + Number((r as any).total_repayment || 0), 0);
    ...
    // GAP-19: memo_number is auto-generated by DB trigger — do NOT pass it from client
